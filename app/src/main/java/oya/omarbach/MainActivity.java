...
public class MainActivity extends Activity implements SensorEventListener {
    ...
    int s;
    double magnitude;
    ArrayList<Double> AverageCycle = new ArrayList<>();
    ...
    public ArrayList<Double> interpolate(ArrayList<Double> samples){
        for(int i = 1;i<samples.size()-1;i++){
            double x = samples.get(i-1)+(((10000000*i)-time.get(i-1))*((samples.get(i+1)-samples.get(i-1))/(time.get(i+1)-time.get(i-1))));
            samples.set(i,x);
        }
